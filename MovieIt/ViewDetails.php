...
<div class="container">
	...
	if (($_SESSION['isUser'])== TRUE)	
		...
		echo"
			...
			<form method='POST' action='".setComment($link)."'>
			<input type='hidden' name='userName' value='$isName' >
			<input type='hidden' name='movieId' value='$movieid' >
			<input type='hidden' name='date' value='".date('Y-m-d H:i:s',time())."' >
			<textarea name= 'message'></textarea><br><br>
			<button class='textbutton' name='commentSubmit' type='submit'> comment </button>
			</form>";
		}
		else
		{
			...
			<form method='POST' action='".setComment($link)."'>
			<input type='hidden' name='userName' value='annonymous' >
			<input type='hidden' name='movieId' value='$movieid' >
			<input type='hidden' name='date' value='".date('Y-m-d H:i:s')."' >
			<textarea name= 'message'></textarea><br><br>
			<button class='textbuttonComment' name='commentSubmit' type='submit'> Comment </button>
			</form>";
		}
		/*comment show*/
		echo'<h3 class="commentsHeader">Join the discussion: </h3>
			<hr style="border-top: 1px solid #ccc; background: transparent;">';
		$commentShowSql="select userName,message,date from comment where movieId='{$movieid}'";
		$result=mysqli_query($link,$commentShowSql);
		while($row=mysqli_fetch_array($result))
		{
			echo "<div class='comment-box'><p>";
				echo "<i>Name:</i>  ".$row['userName']. "<br><br>";
				echo "<i>Date/time:</i>  ".$row['date']. "<br><br><br>";
				echo "Comment:  ".$row['message'];
			echo"</p></div>";
